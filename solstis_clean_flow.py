#!/usr/bin/env python3
# Solstis Voice Assistant with Clean Flow and Dual Wake Word Support
# Natural conversation flow with SOLSTIS and STEP COMPLETE wake words
...
waiting_for_step_complete = False
            ...
            while len(buf) - offset >= frame_bytes:
                ...
                if r >= 0:
                    if r == 0:
                        ...
                        detected_wake_word = "SOLSTIS"
                    elif r == 1:
                        log("STEP COMPLETE wake word detected! 🔊")
                        detected_wake_word = "STEP_COMPLETE"
                    wake_word_detected = True
                    break
            leftover = buf[offset:]

        # Handle STEP COMPLETE wake word
        if detected_wake_word == "STEP_COMPLETE":
            global waiting_for_step_complete
            if waiting_for_step_complete:
                log("✅ Step complete acknowledged, continuing procedure")
                waiting_for_step_complete = False
                return "STEP_COMPLETE"
            else:
                log("⚠️ STEP COMPLETE detected but not waiting for it, ignoring")
                return None

        # Now capture audio until speech pause (for SOLSTIS wake word)
...
async def main():
    ...
    log(f"🩺 Solstis Clean Flow Voice Assistant starting...")
    ...
    try:
        # Main conversation loop - clean and simple
        while True:
            # Wait for wake word and capture audio
            log("👂 Waiting for wake word...")
            audio_data = await asyncio.to_thread(capture_audio_after_wakeword)
            
            # Handle STEP COMPLETE wake word
            if audio_data == "STEP_COMPLETE":
                log("✅ Step complete acknowledged")
                continue
            
            if not audio_data or len(audio_data) < int(OUT_SR * 2 * 0.1):   # ~100 ms min
            ...
            # Handle first interaction with opening message
            if first_interaction:
                first_interaction = False
                opening_message = f"Hey {USER_NAME}. I'm SOLSTIS and I'm here to help. If this is a life-threatening emergency, please call 9-1-1 now. Otherwise, is there something I can help you with?"
                say(opening_message)
                
                # Check if user wants help
                if user_message.lower() in ["no", "nothing", "i'm fine", "no thanks"]:
                    say("OK, if you need me for any help, say SOLSTIS to wake me up.")
                    continue
            ...
            # Check if response indicates user action is required
            response_lower = response_text.lower()
            if any(phrase in response_lower for phrase in [
                "let me know when", "when you're done", "when you're ready", 
                "say step complete", "tell me when", "let me know"
            ]):
                waiting_for_step_complete = True
                log("⏳ Waiting for step completion")
