...
async def main():
    ...
    try:
        ...
        log(f"🎵 Opening Greeting: Converting to speech...")
        audio_data = text_to_speech(opening_message)
        if audio_data:
            log(f"🎵 Opening Greeting: TTS generated {len(audio_data)} bytes, starting playback")
            start_speak_pulse()
            play_audio(audio_data)
            stop_speak_pulse()
            log(f"🎵 Opening Greeting: Playback completed successfully")
        else:
            log(f"🎵 Opening Greeting: No audio data generated, skipping playback")
